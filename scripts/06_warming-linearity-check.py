...
# %%
for scenario in ['ssp119', 'ssp126', 'ssp245', 'ssp370', 'ssp585']:
    os.makedirs(f'../plots/diagnostics/toa/{scenario}', exist_ok=True)
    for model in data:
        if scenario not in data[model]:
            continue
        n_runs = len(data[model][scenario])
        ...
        fig, ax = pl.subplots(grid_size, grid_size, figsize=(16, 12), squeeze=False)
        for irun, run in enumerate(data[model][scenario]):
            ax[irun//grid_size, irun%grid_size].plot(np.arange(1850, 1850+len(data[model][scenario][run]['toa_fitted'])), data[model][scenario][run]['toa_fitted'], color='k')
            ax[irun//grid_size, irun%grid_size].plot(np.arange(1850, 1850+len(data[model][scenario][run]['toa_truth'])), data[model][scenario][run]['toa_truth'], color='b')
            ax[irun//grid_size, irun%grid_size].set_title(f'{model}, {scenario}, {run}')
        fig.tight_layout()
        pl.savefig(f'../plots/diagnostics/toa/{scenario}/{model}.png')
...
# %%
for scenario in ['ssp119', 'ssp126', 'ssp245', 'ssp370', 'ssp585']:
    os.makedirs(f'../plots/diagnostics/forcing/{scenario}', exist_ok=True)
    for model in data:
        if scenario not in data[model]:
            continue
        n_runs = len(data[model][scenario])
        ...
        fig, ax = pl.subplots(grid_size, grid_size, figsize=(16, 12), squeeze=False)
        for irun, run in enumerate(data[model][scenario]):
            ax[irun//grid_size, irun%grid_size].plot(np.arange(1850, 1850+len(data[model][scenario][run]['forcing_fitted'])), data[model][scenario][run]['forcing_fitted'], color='k')
            ax[irun//grid_size, irun%grid_size].set_title(f'{model}, {scenario}, {run}')
        fig.tight_layout()
        pl.savefig(f'../plots/diagnostics/forcing/{scenario}/{model}.png')
